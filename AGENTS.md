...
This repo is a [cookiecutter](https://cookiecutter.readthedocs.io/) template, not a package in its own right:
...
This repo has no release/versioning process of its own (it isn't published anywhere), so it stays trunk-based: work off `main` directly, PR review before merging. There's no `develop` branch.
